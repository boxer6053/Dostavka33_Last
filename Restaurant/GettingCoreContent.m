            ...
            while(counter <  values.count)
            {
                ...
                for(int i=0;i<keys.count;i++)
                {
                    ...
                    if ([editAttrinbuteWithUnderBar isEqualToString:@"idPicture"] || [editAttrinbuteWithUnderBar isEqualToString:@"underbarid"] || [editAttrinbuteWithUnderBar isEqualToString:@"idProduct"] || [editAttrinbuteWithUnderBar isEqualToString:@"version"] || [editAttrinbuteWithUnderBar isEqualToString:@"isOrder"] || [editAttrinbuteWithUnderBar isEqualToString:@"terrace"] || [editAttrinbuteWithUnderBar isEqualToString:@"parking"] || [editAttrinbuteWithUnderBar isEqualToString:@"seatsNumber"] || [editAttrinbuteWithUnderBar isEqualToString:@"hit"] ||
                        [editAttrinbuteWithUnderBar isEqualToString:@"carbs"] || [editAttrinbuteWithUnderBar isEqualToString:@"calories"] || [editAttrinbuteWithUnderBar isEqualToString:@"weight"] ||[editAttrinbuteWithUnderBar isEqualToString:@"protein"]  || [editAttrinbuteWithUnderBar isEqualToString:@"fats"] ||[editAttrinbuteWithUnderBar isEqualToString:@"idIngredient"] ||[editAttrinbuteWithUnderBar isEqualToString:@"idSize"] ||[editAttrinbuteWithUnderBar isEqualToString:@"isMultisize"] ||[editAttrinbuteWithUnderBar isEqualToString:@"isTopping"] ||[editAttrinbuteWithUnderBar isEqualToString:@"idThumbnail"] ||[editAttrinbuteWithUnderBar isEqualToString:@"isDefault"])
                    {
...
- (NSArray *)fetchArrayOfSizesByIds:(NSArray *)SizesIds withDefaultLanguageId:(NSString *)languageId
{
    ...
    {
        SizeStruct *size = [[SizeStruct alloc]init];
        [request setEntity:[NSEntityDescription entityForName:@"Sizes" inManagedObjectContext:self.managedObjectContext]];
        request.predicate = [NSPredicate predicateWithFormat:@"underbarid == %@", [SizesIds objectAtIndex:i]];
        NSArray *arrayOfDictionaries= [self.managedObjectContext executeFetchRequest:request error:&error];
        size.sizeId = [[arrayOfDictionaries objectAtIndex:0] valueForKey:@"underbarid"];
        size.value = [[arrayOfDictionaries objectAtIndex:0] valueForKey:@"value"];
        size.price = [[arrayOfDictionaries objectAtIndex:0] valueForKey:@"price"];

        [request setEntity:[NSEntityDescription entityForName:@"Sizes_translation" inManagedObjectContext:self.managedObjectContext]];
        request.predicate = [NSPredicate predicateWithFormat:@"idLanguage == %@ && idSize == %@",languageId, [SizesIds objectAtIndex:i]];
        NSArray *arrayOfTexts = [self.managedObjectContext executeFetchRequest:request error:&error];
        size.name = [[arrayOfTexts objectAtIndex:0] valueForKey:@"nameText"];
        [arrayOfsizes addObject:size];
    }
    // Save the context.
    if (![self.managedObjectContext save:&error])
    {
...
- (NSArray *)fetchAllSizeIdWithProductId:(NSNumber *)productId
{
    NSFetchRequest * request = [[NSFetchRequest alloc] init];
    [request setEntity:[NSEntityDescription entityForName:@"Multisize" inManagedObjectContext:self.managedObjectContext]];
    request.predicate = [NSPredicate predicateWithFormat:@"idProduct == %@", productId];
    ...
    {
        id object = [arrayOfDictionaries objectAtIndex:i];
        if ([[object valueForKey:@"idProduct"] isEqual: productId])
        {
            [arrayOfSizes addObject: [object valueForKey:@"idSize" ]];
        }
    }
...
- (NSArray *)fetchArrayOfIngredientsByIds:(NSArray *)ingredientsIds withDefaultLanguageId:(NSString *)languageId
{
    ...
    {
        IngredientDataStruct *ingredient = [[IngredientDataStruct alloc]init];
        [request setEntity:[NSEntityDescription entityForName:@"Ingredients" inManagedObjectContext:self.managedObjectContext]];
        request.predicate = [NSPredicate predicateWithFormat:@"underbarid == %@", [ingredientsIds objectAtIndex:i]];
        NSArray *arrayOfDictionaries= [self.managedObjectContext executeFetchRequest:request error:&error];
        ingredient.price = [[arrayOfDictionaries objectAtIndex:0] valueForKey:@"price"];
        ingredient.weight = [[arrayOfDictionaries objectAtIndex:0] valueForKey:@"weight"];
        ingredient.ingredientId = [[arrayOfDictionaries objectAtIndex:0] valueForKey:@"underbarid"];
        
        ingredient.idPicrure = [[arrayOfDictionaries objectAtIndex:0] valueForKey:@"idPicture"];
        ingredient.image = [UIImage imageWithData:[self fetchPictureDataByPictureId:[NSString stringWithFormat:@"%@", ingredient.idPicrure]]];
        NSLog(@"%@", ingredient.idPicrure);
        [request setEntity:[NSEntityDescription entityForName:@"Ingredients_translation" inManagedObjectContext:self.managedObjectContext]];
        request.predicate = [NSPredicate predicateWithFormat:@"idLanguage == %@ && idIngredient == %@",languageId, [ingredientsIds objectAtIndex:i]];
        NSArray *arrayOfTexts = [self.managedObjectContext executeFetchRequest:request error:&error];
        ingredient.name = [[arrayOfTexts objectAtIndex:0] valueForKey:@"nameText"];
        ingredient.descriptionText = [[arrayOfTexts objectAtIndex:0] valueForKey:@"descriptionText"];
        [arrayOfIngredients addObject:ingredient];
    }
    // Save the context.
    if (![self.managedObjectContext save:&error])
    {
...
- (NSArray *)fetchAllDefaultIngredientsIdWithProductId:(NSNumber *)productId
{
    NSFetchRequest * request = [[NSFetchRequest alloc] init];
    [request setEntity:[NSEntityDescription entityForName:@"Toppings" inManagedObjectContext:self.managedObjectContext]];
    request.predicate = [NSPredicate predicateWithFormat:@"idProduct == %@ && isDefault == 1", productId];
...
- (NSArray *)fetchAllIngredientsIdWithProductId:(NSNumber *)productId fromEntity:(NSString *)entity
{
    NSFetchRequest * request = [[NSFetchRequest alloc] init];
    ...
    for (int i=0; i<arrayOfDictionaries.count; i++)
    {
        ...
        if ([[object valueForKey:@"idProduct"] isEqual: productId])
        {
            [arrayOfIngredients addObject: [object valueForKey:@"idIngredient" ]];
        }
    }
...
- (void) deleteIngredientByProductId:(NSNumber *)productId
{
    NSManagedObjectContext * context = self.managedObjectContext;
    NSFetchRequest *fetchRequest = [[NSFetchRequest alloc] init];
    NSEntityDescription *entity = [NSEntityDescription entityForName:@"Cart_Products_Ingredients" inManagedObjectContext:context];
    [fetchRequest setEntity:entity];
    ...
    for (int i = 0; i < items.count; i++)
    {
        ...
        //    [context deleteObject:[items objectAtIndex:i]];
        if ([[[items objectAtIndex:i] valueForKey:@"idProduct"] intValue] == productId.intValue)
        {
            [context deleteObject:[items objectAtIndex:i]];
        }
    }
    ...
    if (![context save:&error]) {
        NSLog(@"Error deleting %@ - error:%@", @"Cart_Products_Ingredients", error);
    }
}

- (void)SaveProduct:(NSNumber *)productId withIngredient:(NSNumber *)ingredientId
{
    NSFetchRequest * request = [[NSFetchRequest alloc] init];
    [request setEntity:[NSEntityDescription entityForName:@"Cart_Products_Ingredients" inManagedObjectContext:self.managedObjectContext]];
    
    NSError *error;
    NSManagedObject *objectToInsert = [NSEntityDescription insertNewObjectForEntityForName:@"Cart_Products_Ingredients" inManagedObjectContext:self.managedObjectContext];
    
    [objectToInsert setValue:productId forKey:@"idProduct"];
    [objectToInsert setValue:ingredientId forKey:@"idIngredient"];
              ...
              withDiscountValue:(float)discountValue
                   ...
                   withCalories:(NSNumber *)calories
                    isFavorites:(BOOL)isFavorites
                      isTopping:(BOOL)isTopping
                    isMultisize:(BOOL)isMultisize
                          isHit:(BOOL)hit
...
//                int curInt = [[objectToUpdate valueForKey:@"count"] intValue] + countOfProducts;
                ...
                [objectToUpdate setValue:[NSNumber numberWithFloat:cost] forKey:@"cost"];
                [objectToUpdate setValue:sizeId forKey:@"sizeId"];
            }
    ...
    if (!allreadyInEntity)
    {
        ...
        [objectToInsert setValue:[NSNumber numberWithBool:isTopping] forKey:@"isTopping"];
        [objectToInsert setValue:[NSNumber numberWithBool:isMultisize] forKey:@"isMultisize"];
        [objectToInsert setValue:[NSNumber numberWithBool:hit] forKey:@"hit"];
        [objectToInsert setValue:sizeId forKey:@"sizeId"];
