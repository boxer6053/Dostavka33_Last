...
//        sNSLog(@"Second query end. For begin");
        for(int i=0;i<data.count;i++)
        {
            if(i%2==0)
            {
                ...
                dataStruct.isMultisize = [[data objectAtIndex:i] valueForKey:@"isMultisize"];
                dataStruct.isTopping = [[data objectAtIndex:i] valueForKey:@"isTopping"];
...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    [segue.destinationViewController setProduct:[self.arrayData objectAtIndex:self.selectedRow.integerValue] isFromFavorites:NO isFromCart:NO];
}
