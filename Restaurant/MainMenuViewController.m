...
-(NSMutableArray *)arrayOfObjects
{
    if (!_arrayOfObjects || _arrayOfObjects.count != [self.db fetchAllProductsIdAndTheirCountWithPriceForEntity:@"Cart"].count)
    {
        ...
        for (int i = 0; i <array.count; i++)
        {
            ...
            [productStruct setSizeId:[[array objectAtIndex:i] valueForKey:@"sizeId"]];
...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    else if([segue.identifier isEqualToString:[[NSUserDefaults standardUserDefaults] objectForKey:@"typeOfViewFavorites"]])
    {
        ...
        {
            [[segue destinationViewController] setProduct:[self.arrayOfObjects objectAtIndex:self.selectedRow.integerValue] isFromFavorites:NO isFromCart:YES];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if(self.isCartMode)
    {
        if (indexPath.section == 0)
        {
            ...
            NSString *price = [formatter stringFromNumber:[NSNumber numberWithFloat:(productStruct.price.floatValue * productStruct.count.floatValue * [[[NSUserDefaults standardUserDefaults] objectForKey:@"CurrencyCoefficient"] floatValue])]];
        ...
        else
        {
            ...
            for (int i = 0; i < self.arrayOfObjects.count; i++)
            {
                ...
                sum = sum + ([[formatter numberFromString:[formatter stringFromNumber:[NSNumber numberWithFloat:(productDataStruct.price.floatValue * productDataStruct.count.floatValue * [[[NSUserDefaults standardUserDefaults] objectForKey:@"CurrencyCoefficient"] floatValue])]]] floatValue]);
                if (productDataStruct.discountValue.floatValue != 0)
                {
                    sumWithDiscounts = sumWithDiscounts + (productDataStruct.price.floatValue * productDataStruct.count.floatValue * (1 - productDataStruct.discountValue.floatValue) * [[[NSUserDefaults standardUserDefaults] objectForKey:@"CurrencyCoefficient"] floatValue]);
                    isWithDiscount = YES;
                }
                else
                {
                    sumWithDiscounts = sumWithDiscounts + ([[formatter numberFromString:[formatter stringFromNumber:[NSNumber numberWithFloat:(productDataStruct.price.floatValue * productDataStruct.count.floatValue * [[[NSUserDefaults standardUserDefaults] objectForKey:@"CurrencyCoefficient"] floatValue])]]] floatValue]);
                }
